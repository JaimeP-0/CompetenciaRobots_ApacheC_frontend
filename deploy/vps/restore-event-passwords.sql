-- Restaura contraseñas, nombres y asignaciones del evento.
-- Equivalente a seed-users.sql pero sin tocar ids ni crear usuarios nuevos.
-- Aplicar: npm run vps:restore-passwords
...
FROM (
    VALUES
        ...
        ('guillermo.iglesias', 'Ing. Guillermo Elías Iglesias López',        'juez',      '$2b$10$3N41d3Xawe7noFwxhuri6OUugwelbgtkOGye7FSZEYAyaKQAFTQiC'),
...
) AS s(username, name, role, password_hash)
...
SELECT u.id, u.username, u.name, u.role, left(u.password_hash, 27) || '…' AS hash_prefix
FROM user_account u
ORDER BY u.id;
