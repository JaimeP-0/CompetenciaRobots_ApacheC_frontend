# -*- coding: utf-8 -*-
"""Aplica deploy/vps/update-user-names.sql en Postgres del VPS."""
import os
import sys
...
SQL_FILE = ROOT / "deploy" / "vps" / "update-user-names.sql"
REMOTE = "/var/www/competencia-robots/deploy/vps/update-user-names.sql"
...
def main():
    ...
    print("==> Actualizando nombres en user_account…", file=sys.stderr)
    ...
    verify = (
        ...
        f'"SELECT username, name, role FROM user_account '
        f'WHERE username NOT IN (\'admin\', \'dev\', \'visitante\') '
        f'ORDER BY role, username;"'
    )
    _, stdout, _ = c.exec_command(verify)
    print(stdout.read().decode())
    c.close()
    print("Nombres actualizados.", file=sys.stderr)
