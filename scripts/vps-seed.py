# -*- coding: utf-8 -*-
"""Aplica deploy/vps/seed-data.sql en Postgres del VPS."""
import os
import sys
...
SEED = ROOT / "deploy" / "vps" / "seed-data.sql"
REMOTE = "/var/www/competencia-robots/deploy/vps/seed-data.sql"
...
def main():
    ...
    verify = (
        ...
        f'"SELECT c.name, COUNT(t.id) AS teams FROM category c '
        f'LEFT JOIN team t ON t.category_id = c.id GROUP BY c.name ORDER BY c.name;"'
    )
    _, stdout, _ = c.exec_command(verify)
    print(stdout.read().decode())
    c.close()
    print("Seed aplicado.", file=sys.stderr)
